...
\section{Theory}	
	...
	\subsection{Viscosity}
		...
		For example if the side chain has a substituent which can make H-Bonds the viscosity gets higher.
		In total it can be stated, the stronger the intramolecular forces the higher the viscosity.\\

		The third one is the influence of the solvent. Because different solvents have different properties.
		If the solvent is better the tangle gets more swallen and the viscosity gets higher.\\

		The fourth is the influence of the concentration. This means with different concentration the viscosity changes.
		This is due to higher concentration leads to more intermolecular forces which raises the viscosity.\\
		...
		The temperature also destroys the intermolecular forces which leads to a less tangeled structure wich also leads to a less rigid polymer.
		This results in a lower viscosity.\\
...
\section{Results}
		...
		\begin{table}[H]
			...
			\begin{tabular}{c|ccccc|c}
				\toprule
				Sampleconcentration [$10^{-3} \, \frac{\text{g}}{\text{ml}}$]& Transit time 1 [s] & Transit time 2 [s] & Transit time 3 [s] & Transit time 4 [s] & Transit time 5 [s] & Average transit time [s]\\ 
				\midrule
		...
		The transit time for the sample with a concentration of 0 is used as the standard time $t_0$. This results in a specific viscosity 
		of 
		...
		for the sample with a concentration of $1.25 \cdot 10^{-3}$. The specific viscosity is then divided by the concentration $c$, yielding the 
		...
		\begin{table}[H]
			\centering
			\caption{Specific and reduced viscosity at different polymer concentrations.}
		...
		$c = \SI{0}{\frac{g}{ml}}$. The graph is shown in Figure \ref{reducedaufconcplot}.
			...
			\begin{figure}[H]
				\centering
				\includegraphics{Bilder/reducedaufconc.png}
				\caption{Reduced viscosity $\eta_\text{red}$ plotted against concentration $c$ using a regression line $\eta_\text{red} = \SI{2.0338}{\frac{ml^2}{g^2}} \cdot c + \SI{0.0557}{\frac{ml}{g}}$.}
				\label{reducedaufconcplot}
			\end{figure}

		If a concentration of 0 is substituted into the linear equation, only the y-intercept remains, which in itself yields a viscosity 
		number of $[\eta] = \SI{0.0557}{\frac{ml}{g}}$.\\
		...
		\begin{equation*}
			M_\eta = \sqrt[0.71]{\frac{\SI{0.0557}{\frac{ml}{g}}}{\SI{0.012}{\frac{ml}{g}}}} = 8.689 \text{.}
		\end{equation*}
