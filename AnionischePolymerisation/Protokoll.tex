...
\section{Auswertung}
	...
	So konnte eine Molmasse von M$_\text{n} = \SI{21660}{g/mol}$ und von M$_\text{w} = \SI{27060}{g/mol}$ bestimmt werden.
	...
	Die theoretische Molmasse ist um einiges geringer. 
	Was aus dem nicht mitreagierendem Initiator folgt.
...
\section{Fragen}
	...
	\subsection{Frage 1}
		...
		Die Molmasse lässt sich durch folgende Gleichung beschreiben:
			...
			\begin{equation}
				M_\text{Polymer} = \frac{m_\text{m}}{n_\text{i}}
			\end{equation}

	\subsection{Frage 2}

		Die Stoffmenge an Methanol zum abbrechen ist dieselbe wie die des Initiators.

	\subsection{Frage 3}

		Da Toluol am ahnlichsten zu Styrol ist und sich styrol darin gut löst. 
		Im vergleich zu Benzol ist es weitestgehend ungefährlich im umgang. 

	\subsection{Frage 4}

		Ist in den Abbildungen [\ref{Initiierung}- \ref{Abbruch}] zu sehen.
	...
	\subsection{Frage 7}
		...
		\begin{figure}[H]
			...
			\caption{Die Abbildung zeigt die ringöffnende Polymerisation von Caprolactam zu Nylon-6.}
			\label{Nylon6}
		\end{figure}

	\subsection{Frage 8}

		Zum herstellen des SBS Blockcopolymer ist ein ähnliches Vorgehen wie im Versuch möglich. 
		Jedoch wird bei der herstellung von SBS zuerst Styrol, gefolgt von Butadien und anschließend erneut Styrol zugegeben.
