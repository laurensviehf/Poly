...
\section{Auswertug}
	...
	\subsection{Standardmessung der Ferrocen/Ferrocenium Elektrode}
		...
		Dabei wurde eine Ferrocen/Ferrocenium (Fe/Fef$^+$) elektrode verwendet mit einer Ag-Elektrode als Gegenelektrode.
		Das aufgenommene CV ist in Abbildung \ref{CV-Fe} aufgezeigt.
			\begin{figure}[H]
				\centering
				\includegraphics[scale = 0.5]{Bilder/Ferrocen.png}
				\caption{Die Abbildung zeigt das CV von einer Ferrocen/Ferrocenium Elektrode mit einer Ag-Elektrode als Gegenelektrode.
		...
		werden diese beiden subtrahiert erhält man den $E_{1/2}$ Wert. Welcher in den Folgenden Versuchen als Standardpotential verwendet werden kann.
		Dabe wird nach folgender Gleichung vorgegangen.
	...
	\subsection{Elektropolymerisation von EDOT}	
		...
		Der folgende Graph \ref{EDOT} zeigt das CV einer Polymerisation von EDOT zu PEDOT.
		Durch analysieren des Graphen werden nicht nur die drei Oxidationspeaks rechts erkennbar sonder auch ein kleiner Peak im Bereich von $\SI{0,7}{V}$ zu erkennen.
		Welche vorallem im dritten zyklus deutlich wird.
		Dieser lässt sich durch bereits vorhandenes PEDOT erklären welches oxidiert wird. 
		Dabei ist wichtig, dass PEDOT ein geringeres Oxidationspotential hat und daher weiter links kommt.  
			...
			\end{figure}
\newpage
		In folgender Abbildung \ref{EDOT-Mechanismus} ist der Mechanismus dieser PEDOT Polymerisation aufgezeigt. 
		Bei diesem findet eine Bildung eines Radikals statt welches, in diesem Fall, mit einer identischen spezies Rekombiniert.
		Anschlißend findet eine Abgabe von Protonen statt welche eine rearomatisierung zufolge hat.
	...
	\subsection{Bestimmung der HOMO- und LUMO-Niveaus}
		...
		Ziel war es hier die elektrochemische Bandlücke zwischen HOMO und LUMO zu bestimmen.
		Dafür musste zunächst die jeweiligen Energien benötigt. Diese wurden über Cyclovoltametrie bestimmt.
		Diese CV sind in Abb. \ref{CV-P3HT} zu sehen, dabei ist die Oxidation oben und die Reduktion unten.
		Jeweils darüber lässt sich die Energie des HOMO (Oxidation) und die des LUMO (Reduktion) bestimmen.
			...
			\begin{figure}[H]
				\centering
				\includegraphics[scale = 0.5]{Bilder/P3HT_Full_Onsets.png}
				\caption{}
				\label{}
			\end{figure}
	...
	\subsection{Elektrochrome Eigenschaften von PEDOT}
		...
		Die foilgenden drei Abbildungen zeigen die Stromstäke aufgetragen auf die Zeit für die unterschiedlichen Umkehrintervalle bei der Elektrochromie des PEDOTS.
		Es konnte bei allen drei ein Farbwechsel festgestellt werden.
		Die spanne der unterschiedlichen verfärbungen wären immer ähnlich lang wie die des Intervalls.
		Es wird unteranderem deutlich, dass bei verlängern des Intervalls die Stromstärke eher wieder auf die $\SI{0}{A}$ absinkt. 
		Was durch eine vollständigere Reduktion des PEDOT erklären lässt.
