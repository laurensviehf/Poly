...
def calc_tangents(v, i, baseline_v_range, peak_v_range, target_peak_idx=0):
    ...
    # Wir suchen NUR nach positiven Steigungen (Wichtig: kein np.abs() mehr)
...
# Baseline von 0.0 bis 0.15V. Peak-Suche von 0.2 bis 1.2V. Index 0 wählt den ersten positiven Anstieg.
m_b_h, c_b_h, m_p_h, c_p_h, v_on_h, i_on_h = calc_tangents(v_homo_sw, i_homo_sw, [0.0, 0.15], [0.1, 1.2], 0)

# Für LUMO:
v_lumo_sw, i_lumo_sw = get_forward_sweep(cycle3_lumo, 'down')
m_b_l, c_b_l, m_p_l, c_p_l, v_on_l, i_on_l = calc_tangents(v_lumo_sw, i_lumo_sw, [-0.8, 0.0], [-1.8, -1.0], 0)
...
v_line_h = np.linspace(-0.2, 1.2, 100) 
ax1.plot(v_line_h, (m_b_h * v_line_h + c_b_h) * 1e6, 'k--', label='Erweiterte Baseline')
ax1.plot(v_line_h, (m_p_h * v_line_h + c_p_h) * 1e6, 'g--', label='Peak Tangente (Erster Peak)')
...
ax1.set_title('Cyclovoltammetrie: HOMO', fontsize=14)
...
v_line_l = np.linspace(-2.1, 0.2, 100)
...
ax2.set_xlim(-2.2, 0.2)
ax2.set_ylim(cycle3_lumo['WE(1).Current (A)'].min()*1e6*1.1, 3)
ax2.set_title('Cyclovoltammetrie: LUMO', fontsize=14)
