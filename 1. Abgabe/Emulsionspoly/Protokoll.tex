...
\section{Einleitung}
	...
	In dem folgenden Versuch soll Styrol in Wasser mit Hilfe von Natriumdodecylhydrogensulfat als Emulgator polymerisiert werden.
	Es wird eine Umsatzbestimmung durch Probenentnahme nach gegeben Zeiten durchgeführt. 
...
\end{table}
	...
	Denn diese sollten auch zunächst stark steigen und dann abflachen. Ein weiterer Grund weshalb der Umsatz so stark anstieg, ist die Probenentnahme. Dabei kann ein 
	Fehler bei der Volumebestimmung vorliegen oder das Gewicht wurde falsch notiert. 
...
\section{Fragen}
	...
	\subsection{Frage 1}
		...
		Dadurch steigt auch der Umsatz, wodurch wiederum die Polymerketten länger werden. 
		Daraus folgt ein Aufquillen der Latexteilchen, und die Viskosität der Emulsion steigt stark an.
		Die dadurch eingeschränkte beweglichkeit der Ketten führt zu einer geringeren Abbruchrate der Polymerisation.
		Das Wachstum der Reaktion ist jedoch nicht eingeschränkt. 
		Durch weiteres Wachstum schaukelt sich dieser Effekt immer weiter hoch.
		Da die Wachstumsreaktion exotherm ist wird die Emulsion immer heißer wodurch auch wieder mehr Initiator zerfällt, welcher auch die Reaktionsgeschwindigkeit steigert.
		Dieser Effekt wird auch Trommsdorff-Effekt oder Gel-Effekt genannt.  

	\subsection{Frage 2}

		Zum einen die Elektrostatische Stabilisierung, dies ist vor allem bei anionischen Emulgatoren der fall. Alle der Latexteilchen besitzen die selbe Ladung, wodurch 
		diese sich gegenseitig abstoßen. Betrachtet man das Natriumdodecylhydrogensulfat hat dieses eine lange polare Kette und ein negativ geladenes Kopfende. Durch 
		die polare und unpolare Seite der Emulgatoren, sind diese zum Wasser und zum Styrol anders ausgerichtet. Die polare Seite ist dabei zum Wasser orientiert, wodurch 
		die Latexteilchen nach außen hin Anionisch sind und so, wie oben erwähnt, sich gegenseitig abstoßen.

			\begin{figure}[H]
				\centering
				\includegraphics[width = \textwidth]{Bilder/Natriumdodecly.png}
				\caption{Die Abbildung zeigt Natriumdodecylhydrogensulfat. \cite{noauthor_dateisodium_2010}}
				\label{Natriumdodecylhydrogensulfat}
			\end{figure}
